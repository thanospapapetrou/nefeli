...
public abstract class XmlContentProvider<T extends XmlContent> {
    protected final Class<? extends XmlAdapter<Element, T>> adapter;

    public static Class<?> getXmlContentClass(final Class<? extends XmlContentProvider<?>> provider) {
        return Arrays.stream(provider.getGenericInterfaces()).filter(ParameterizedType.class::isInstance)
                .map(ParameterizedType.class::cast).filter(t -> t.getRawType() == XmlContentProvider.class)
    ...
    public static String getNamespace(final Class<? extends XmlContentProvider<?>> provider) {
        return getXmlContentClass(provider).getAnnotation(XmlRootElement.class).namespace();
    }

    public static String getLocalName(final Class<? extends XmlContentProvider<?>> provider) {
        return getXmlContentClass(provider).getAnnotation(XmlRootElement.class).name();
    }

    protected XmlContentProvider(final Class<? extends XmlAdapter<Element, T>> adapter) {
